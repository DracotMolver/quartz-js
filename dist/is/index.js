...
function moreOrEqual(value, size, isMoreOnly) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (util.isNumber(value)) {
      console.error('Pass only Object, Array or String in the first paramenter.');
      return;
    }
  }
    ...
    callback = function callback(pwd) {
      if (process.env.NODE_ENV !== 'production') {
        ...
        if (rules.maxLength < rules.minLength) {
          console.error("The maxLength can't be less than the minLenght. It must be more.");
          return;
        }
      }
...
var handler = {
  get: function get(obj, prop, value) {
